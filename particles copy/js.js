...
window.addEventListener("click", () => {
    toogler = !toogler
    canvas.style.display = `${toogler ? "block" : "none"}`
})
...
function checkInnerTargetElement(targetElement, coordinateTargetElement, point){
    if((coordinateTargetElement.x <= point.x && point.x <= coordinateTargetElement.x + coordinateTargetElement.width) && (coordinateTargetElement.y <= point.y && point.y <= coordinateTargetElement.y + coordinateTargetElement.height)){
...
window.addEventListener("click", (event) => {
    ...
    for(let i = 0; i < lengthWeb; i++){
        ...
        let endX =  getRandomArbitrary(coordinateTargetElement.x,coordinateTargetElement.x + coordinateTargetElement.width) + 1
        let endY =  getRandomArbitrary(coordinateTargetElement.y,coordinateTargetElement.y + coordinateTargetElement.height) + 1
...
for(let i = 0; i < lengthWebMouse; i++){
    masWebMouse.unshift({
        ...
        draw(){
            ctx.beginPath()
            ctx.arc(masWebMouse[i].x,masWebMouse[i].y, Math.random() * 1, 0, Math.PI * 2)
            ctx.closePath()
            ctx.fill()
        }
    })
